...
public class ReservationService implements ServiceGeneric<Reservation> {
	 ...
	 * if(reserv.getBeginDate()==null && reserv.getId()!=null ) { return
	 * findIDElements(reserv.getId());
	 * 
	 * } else if(reserv.getBeginDate()!=null && reserv.getId()==null) { return
	 * findBeingDates(reserv.getBeginDate().toString()); } else
	 * if(reserv.getBeginDate()!= null && reserv.getId()!=null) { return
	 * findReservation(reserv.getId(),reserv.getBeginDate()); } return null; }
	 */
	...
	private List<Reservation> findDataRange(LocalDate beginDate, LocalDate endDate) {
		return reservationRepository.findAll().stream()
				.filter(p -> p.getBeginDate().isAfter(beginDate) && p.getEndDate().isBefore(endDate))
				.collect(Collectors.toList());
	}

	/*
	 * private List<Reservation> findReservation(Long id, LocalDate beginDate) {
	 * return reservationRepository.findAll().stream().filter(p ->
	 * (p.getId().equals(id) &&
	 * p.getBeginDate().equals(beginDate))).collect(Collectors.toList()); }
	 * 
	 * private List<Reservation> findBeingDates(String beginDate) { return
	 * reservationRepository.findAll().stream().filter( p ->
	 * p.getBeginDate().equals(beginDate)).collect(Collectors.toList()); }
	 * 
	 * private List<Reservation> findIDElements(Long id) { return
	 * reservationRepository.findAll().stream().filter( p ->
	 * p.getId().equals(id)).collect(Collectors.toList()); }
	 */

	public Reservation findID(Long id) {
		Optional<Reservation> optional = reservationRepository.findAll().stream().filter(p -> p.getId().equals(id))
				.findAny();
	 ...
	 * if(verificateCheckIn(reserva.getId(),debt)) { return true; } return false; }
	 */
